...
vector<vector<int>> bins_from_density(int tot_num_events, int bin_width, double pt_hat_min, double pt_hat_max){
    ...
    double event_density_factor = (tot_num_events / 21500.0) * 150000.0; // 21500 was how many events were made when 150000.0 was the factor
...
int main() {
    ...
    TString filename = "event_fromDensityLarge.root";
    ...
    // using make_bins
    std::vector<vector<int>> pt_hat_bins = bins_from_density(60000, 5, 20, 250); // total desired events, bin width, pt hat min, pt hat max
